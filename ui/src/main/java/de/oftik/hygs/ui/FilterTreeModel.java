...
public class FilterTreeModel extends DefaultTreeModel {
	...
	public void filter(String term) {
		final Enumeration<FilterNode> groups = ((FilterNode) getRoot()).children();
		final String lcTerm;
		if (term == null) {
			lcTerm = null;
		} else {
		...
		while (groups.hasMoreElements()) {
			changed |= visitAll(groups.nextElement(), lcTerm);
		}
		if (changed) {
	...
	private boolean visitAll(FilterNode node, String term) {
		if (node.isLeaf()) {
			...
			final Enumeration<FilterNode> children = node.children();
			final int oldCount = node.getChildCount(true);
			List<FilterNode> changedChildren = new ArrayList<>();
			while (children.hasMoreElements()) {
				final FilterNode child = children.nextElement();
